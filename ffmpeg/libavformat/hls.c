                    ...
                    AVDictionary **opts, AVDictionary *opts2, int *is_http_out)
{
    ...
    if (av_strstart(proto_name, "file", NULL)) {
        ...
        /* libav.js: Rewrite http/https URLs to go through jsfetch protocol */
        av_dict_copy(&tmp, *opts, 0);
        av_dict_copy(&tmp, opts2, 0);
        char *jsfetch_url = av_asprintf("jsfetch:%s", url);
        if (!jsfetch_url) {
            av_dict_free(&tmp);
            return AVERROR(ENOMEM);
        }
        ret = s->io_open(s, pb, jsfetch_url, AVIO_FLAG_READ, &tmp);
        av_free(jsfetch_url);
        av_dict_free(&tmp);
        if (is_http_out)
            *is_http_out = 0;
